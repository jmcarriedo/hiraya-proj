<?php
    require('session.php');
    require('server.php');
    // require('gallery-show-script.php');
    $user_id = $_SESSION['user_id'];

    if(isset($_POST['view'])) {
        $booking_id = $_POST['booking_id'];
        $query = "SELECT DISTINCT galleries.booking_id, galleries.imageName, bookings.booking_id, bookings.customer_id, bookings.eventName, bookings.eventDate
            FROM galleries JOIN bookings ON galleries.booking_id = bookings.booking_id WHERE bookings.booking_id = '$booking_id'";
            ...
            <div class="section__content section__content--p30">
                <div class="container-fluid">
                    ...
                    <table class="user-data table ">
                        <thead>
                            <td>Event Name: <span style="font-weight: bold;" class="text-info"><?php echo mysqli_fetch_array($result)['eventName']; ?></span><br>
                            Event Date: <span style="font-weight: bold;" class="text-info"><?php echo mysqli_fetch_array($result)['eventDate']; ?></span></td>
                        </thead>
                        <tbody>
                            <tr class="ps-3">
                            <td>
                                <?php while($row = mysqli_fetch_array($result)) { ?>
                                    <img src="uploads/<?php echo $row['imageName']; ?>" style="width:150px;"/>
                                <?php } ?> 
                            </td>
                            </tr>     
                            <tr> <td><a href="gallery.php" class="text-secondary"><i class="fas fa-reply"></i> Back to Gallery List</a></td></tr>
                        </tbody>
                    </table>            
